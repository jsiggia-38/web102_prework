...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        const display = `<div class = "game">
        <div class = "game-img">
        <img src = "${games[i].img}" height="200" width="300" />
        </div>
        <h1>${games[i].name}</h1>
        ...
        //gamesContainer.innerHTML = <p>What is going on?</p>
...
let totalFundedPledged = fundedGames.reduce( (acc, game) => {
    return acc + game.pledged;
}, 0);

let displayStr = `A total of $${totalFundedPledged.toLocaleString('en-US')} has been raised for ${fundedGames.length} games. Currently, ${unFundedGames.length == 1 ? " 1 game remains " : numUnfundedGames + " games remain "} unfunded. We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
let pElement = document.createElement('p');
pElement.innerHTML = `<p>${displayStr}</p>`;
descriptionContainer.append(pElement);
